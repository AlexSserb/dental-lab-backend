...
class Command(BaseCommand):
    def handle(self, **options):
        now = datetime.now()
        start = datetime(year=now.year, month=now.month, day=now.day, hour=9, minute=0, second=0, microsecond=0)
        ...
        # completed before tasks
        datas = [
            # region product 1
            Operation(
                operation_type_id=OperationTypes.MODELS,
                operation_status_id=OperationStatuses.COMPLETED,
