...
class OrdersTest(TestCase):
    ...
    def test_get_orders_correct(self):
        order = Order.objects.create(user=self.user, discount=5, status=OrderStatus.objects.get(number=3))
    ...
    def test_get_orders_for_physician_correct(self):
        ...
        # Order for new user
        order1 = Order.objects.create(user=user, discount=5, status=OrderStatus.objects.get(number=3))
        # Order for main user
        order2 = Order.objects.create(user=self.user, discount=10, status=OrderStatus.objects.get(number=1))
    ...
    def check_products_created_correctly(self, products: list[Product]) -> tuple:
        ...
        product1, product2 = (products[0], products[1]) if products[0].amount == 4 else (products[1], products[0])
    ...
    def test_confirm_order(self):
        ...
        self.assertTrue(product2.product_status.name == ProductStatus.objects.get(number=1).name)
